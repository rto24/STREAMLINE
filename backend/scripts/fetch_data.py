...
def fetch_user_data(access_token: str, limit=5, output_file: str = "backend/data/train.json"):
  ...
  reformatted_data = [
    {
      "input": f"User likes tracks: {', '.join(combined_track_data)}. User's top artists: {', '.join(top_artists)}. User's top genres: {', '.join(top_genres)}. Recommend {limit} songs that match the user's taste in music.",
